...
def agent_flow_quick(user_q):
    ...
    Steps:
    1. Retrieve search links based on the user's query keyword.
    2. Analyze each link's relevance using the WebSense agent.
    3. Filter relevant links and scrape their content.
    4. Use the FusionBot agent to generate an answer with images based on the scraped content.
    ...
    """
    # Step 1: Fetch search links based on the query
    log_message("🔍 Fetching related page links...")
    shared_flow_state_str.value = "🔗 Fetching related page links..."  # Update flow state for the UI
    
    search_links = fetch_links_with_keyword(user_q)
    log_message(f"🔗 Links fetched: {search_links}")

    # Step 2: Analyze relevance of each link
    shared_flow_state_str.value = "📊 Analyzing related pages with WebSense agent..."  # Update flow state for the UI
    relevant_links = []
    for link in search_links:
        # Fetch the page title
        title = fetch_page_title(link)
        if not title or "Request failed" in title or "Error processing" in title:
            log_message(f"⚠️ Failed to fetch title for link: {link}")
            continue  # Skip this link if title fetch fails

        # Evaluate relevance using the WebSense agent
        is_relevant = websense_agent.generate_response(f"问题是 {user_q}, 网页标题是 {title}")
        if is_relevant.strip().lower() == "true":
            log_message(f"✅ Relevant link found by {websense_agent.name}: {link} (Title: {title})")
            relevant_links.append(link)
        else:
            log_message(f"❌ Irrelevant link skipped by {websense_agent.name}: {link} (Title: {title})")

    log_message(f"📋 Relevant links: {relevant_links}")

    # Step 3: Process relevant links
    shared_flow_state_str.value = "🛠️ Processing related pages..."  # Update flow state for the UI

    # Ensure the cache directory is clean before processing
    if os.path.exists("quicksearch_cache"):
        shutil.rmtree("quicksearch_cache")  # Remove all files in the directory
    os.makedirs("quicksearch_cache/rawdata", exist_ok=True)  # Recreate the directory

    with multiprocessing.Pool(processes=multiprocessing.cpu_count()) as pool:
        list(tqdm(pool.imap(process_url, relevant_links), total=len(relevant_links)))
    log_message("✅ Completed web scraping process.")

    # Step 4: Generate answer with FusionBot
    shared_flow_state_str.value = "🤖 Generating an answer with FusionBot..."  # Update flow state for the UI

    context_q = get_context()  # Get the context from the scraped HTML files
